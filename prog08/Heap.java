...
public class Heap <E> extends AbstractQueue<E> {
  ...
  public E poll() {
    ...
    // (Use, getLeft, getRight, isNull, and compare.)
     while(!isNull(getLeft(index)) && compare(index ,getLeft(index))>0 || !isNull(getRight(index)) &&  
     compare(index ,getRight(index))>0) 
     {
  ...
  public boolean remove (Object o) {
    ...
    if(index == 1)
                  return false;
    // If there is only one item, just remove it.
